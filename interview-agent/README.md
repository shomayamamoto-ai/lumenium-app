# 面談支援エージェント v4（音声自動文字起こし版）

モデル／アクター候補者の **スカウト〜面談〜成約〜PDCA** を Claude Code 上で一気通貫にまわすエージェント。
- v3: **Google Calendar / Gmail / Notion** と連携
- v4: **音声 → Whisper → 構造化議事録** の自動パイプライン
...
```
 ...
 └──────────┘   └──────────┘   └─────────────────┘   └──────────┘   └─────────┘
    /dm         /new            raw-audio.m4a 配置   /minutes       /today
    /email-draft /schedule(MCP)  ↓                   /classify      /weekly
                /brief          /transcribe         /remind(MCP)    /compare
                /rehearse         ↓                 /followup
                                raw-notes.md 手書   /email-draft
                                （併用推奨）        /notion-sync
```
...
| コマンド | 何をするか |
|---------|-----------|
| `/transcribe <slug>` | 🎙 音声ファイルを Whisper で文字起こし→PII 自動除去→`transcript.md` 生成 |

セッション開始時、`candidates/` 配下に未処理音声があれば自動で通知するフック (`.claude/settings.json`) も組み込み済み。
...
### C. 面談中
- IC レコーダー / Zoom / ボイスメモで **録音**（本人同意を取得した上で）
- 同時に `candidates/<slug>/raw-notes.md` にも★項目だけ箇条書き（完璧でなくて OK）

### D. 面談直後 → 文字起こし → フォロー開始
```
# 音声ファイルを candidates/<slug>/raw-audio.m4a に配置（mp3/wav/mp4 可）
/transcribe 2026-04-21-tanaka-yuki          # Whisper で自動文字起こし＋PII 除去
/minutes 2026-04-21-tanaka-yuki             # transcript.md と raw-notes.md を併用
...
**面談録音 → タイムスタンプ付き議事録** まで、担当者が打つのは `/transcribe` と `/minutes` の 2 コマンドだけ。
...
├── TRANSCRIPTION-SETUP.md       # Whisper 文字起こしセットアップ
├── .gitignore                   # 個人情報・音声・キャッシュを Git から除外
├── .claude/
│   ├── settings.json            # SessionStart フック登録
│   ├── commands/                # 15 個のスラッシュコマンド
│   │   ├── new.md
│   │   ├── dm.md
│   │   ├── schedule.md          # [MCP] Calendar
│   │   ├── brief.md
│   │   ├── rehearse.md
│   │   ├── transcribe.md        # [音声] Whisper
│   │   ├── minutes.md
│   │   ├── classify.md
│   │   ├── followup.md
...
│   └── hooks/
│       └── detect-pending-audio.sh  # 未処理音声の自動検知
├── scripts/
│   ├── transcribe.sh            # Whisper マルチバックエンドラッパ
│   └── redact-pii.py            # 電話・メール等の自動除去
├── templates/
│   ├── brief.md
│   ├── minutes.md
│   ├── classification.md
...
│       ├── brief.md
│       ├── rehearsal.md
│       ├── raw-audio.m4a         # 音声（gitignore）
│       ├── transcript.md         # 文字起こし（/transcribe で生成、gitignore）
│       ├── transcript.md.raw     # PII 除去前バックアップ（gitignore、手動削除推奨）
│       ├── raw-notes.md
│       ├── minutes.md
│       ├── classification.md
...
- **リアルタイム文字起こし**（面談中にライブで transcript.md 更新）

...
- `/brief` — 候補者ブリーフ生成（面談前）
- `/rehearse` — 面談ロールプレイ台本
- `/transcribe` — 🎙 Whisper で音声→文字起こし（PII 除去付き）
- `/minutes` — 議事録生成（transcript.md と raw-notes.md を併用）
...
## 自動化フック（SessionStart）
`.claude/settings.json` に登録済みの `detect-pending-audio.sh` は、セッション開始時に:
- `candidates/*/raw-audio.*` を走査
- 対応する `transcript.md` が無いものを列挙
- ユーザーに `/transcribe <slug>` の実行を促す
...
└── candidates/
    ...
    └── <slug>/                # 候補者ごとのフォルダ（.gitignore）
        ...
        ├── raw-audio.m4a         # 面談録音（gitignore、本人同意前提）
        ├── transcript.md         # Whisper 文字起こし（/transcribe 出力、PII 除去済み）
        ├── transcript.md.raw     # 除去前バックアップ（レビュー後に手動削除）
...
5. 面談中: **録音しつつ** `raw-notes.md`（チェックリスト）を埋める
6. 面談直後: `raw-audio.m4a` 配置 → `/transcribe` → `/minutes` → `/classify` → `/remind`（Calendar に TODO 登録）→ `/followup`
